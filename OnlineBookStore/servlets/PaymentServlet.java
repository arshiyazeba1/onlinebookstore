...
public class PaymentServlet extends GenericServlet {
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			PreparedStatement ps0 = con.prepareStatement("select * from " + IBookConstants.TABLE_BOOK);
			...
			while (rs0.next()) {
				...
				if (getChecked != null) {
					PreparedStatement ps = con.prepareStatement("select * from " + IBookConstants.TABLE_BOOK + " where "
							+ IBookConstants.COLUMN_BARCODE + "=?");
					ps.setString(1, getChecked);
